          ...
          components: {
            ...
            modules: {
              ...
              global: {
                ...
                header: prefix.modules + 'global/header-simple.html.twig',
                drawer: {
              ...
              widgets: {
                all: {
                  ...
                  item7: {
                    widgetPath: prefix.modules + 'widgets/widget-registered-customers.html.twig'
                  },
                  item8: {
...
var copy = function() {
  return gulp
    .src(path.staticFilesToCopy, {base: 'src'})
    .pipe(gulp.dest('dist/'))
    .pipe(
      browserSync.reload({
        stream: true
...
gulp.task('watch', function() {
  gulp.watch(path.staticFilesToCopy, gulp.series(copy));
  gulp.watch(path.src.sass, gulp.series('sass'));
  gulp.watch(path.src.twig, gulp.series(compileTwig));
  return;
});

gulp.task('build-prod', gulp.series(clean, 'sass-prod', compileTwig, copy, 'build-images'));
gulp.task('build', gulp.series(clean, 'sass', compileTwig, copy));
gulp.task('default', gulp.parallel(gulp.series('build', 'server'), 'watch'));
